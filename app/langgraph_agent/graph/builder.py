...
# Initialize LLM
llm = ChatVertexAI(model="gemini-2.5-flash", temperature=0.9)
    ...
    for tool_call in tool_calls:
        ...
        try:
            ...
            if tool_name == "create_trip":
                ...
                logger.info(
                    f"Creating trip with dates - Start: {
                        tool_args.get('start_date')
                    }, Return: {tool_args.get('return_date')}"
                )

            if tool_name == "check_driver_availability":
                drivers = state_updates.get("all_fetched_drivers", [])
                driver_ids = [driver["id"] for driver in drivers]
                # print(driver_ids)
                tool_args["driver_ids"] = driver_ids
                ...
                logger.info(
                    f"Checking availability with dates - Start: {
            ...
            if tool_name == "create_trip":
                if "error" not in output:
                    ...
                    logger.info(
                        f"Trip created. Stored dates - Start: {
                            state_updates['start_date']
                        }, End: {state_updates['end_date']}"
                    )

                    output["message"] = f"Trip created successfully from {
                        tool_args.get('pickup_city')
                    } to {tool_args.get('drop_city')}. Now I will find drivers for you."
                ...
                if page == 1:
                    ...
                    state_updates["fetch_count"] = (
                        state_updates.get("fetch_count", 0) + 1
                    )
                ...
                logger.info(
                    ...
                    f"Fetched {len(new_drivers)} drivers, total now: {
                        len(state_updates['all_fetched_drivers'])
                    }"
                )
                ...
                output = {
                    "message": "Filters removed. Please search again to see updated results."
                }
            ...
            tool_messages.append(
                ToolMessage(
                    content=f"Error: An unexpected error occurred while running the tool '{
                        tool_name
                    }'.",
