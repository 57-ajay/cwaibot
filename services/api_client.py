...
def get_driver_ids(
    ...
    try:
        ...
        params = {"city": city, "page": page, "limit": limit, "tripId": trip_id}
        ...
        response = requests.get(
            config.GET_PREMIUM_DRIVERS_URL, params=params, timeout=540
        )
...
def create_trip(
    ...
    logger.info(
        f"  Customer: {customer_details.get('name')} (ID: {customer_details.get('id')})"
    )
    ...
    for attempt in range(max_retries):
        try:
            ...
            if response.status_code in [200, 201]:
                ...
                logger.info(
                    f"  ✅ Trip created successfully: {trip_response.get('tripId')}"
                )
                return trip_response
            else:
                logger.error(
                    f"  ❌ API error on attempt {attempt + 1}: {response.status_code}"
                )
                logger.error(f"  Response: {response.text}")
...
def send_availability_request(
    ...
    try:
        ...
        response = requests.post(
            config.SEND_AVAILABILITY_REQUEST_URL, json=payload, timeout=240
        )
